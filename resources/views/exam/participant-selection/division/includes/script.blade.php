<script>
    ...
    function getParticipantList() {
      ...
      let threshold = $("input[name=threshold]").val();
      let obj = {category_id:categoryId, division_id:divisionId, threshold:threshold,
        question_paper_id : $("#form select[name=question_paper_id]").val(),
        _token:'{{ csrf_token() }}'};
      if (categoryId && divisionId && threshold){
        $('.loader').show();
        $.ajax({
          type : 'POST',
          url  : '{{ route('division-wise-result-for-selection') }}',
          data : obj,
